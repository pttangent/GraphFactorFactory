...
def daily_one(part, out_root, late_min, skip, max_rg):
    t=time.time(); out=Path(out_root)/f'date={part.date}'/f'layer_id={part.layer_id}'/f'scale={part.scale}'; op=out/'daily_relation_features.parquet'
    if skip and done(out/'manifest.json'): return {'stage':'daily_relation_features','status':'skipped','date':part.date,'layer_id':part.layer_id,'scale':part.scale}
    df=read_partition(part.base,['date','decision_time','layer_id','scale','level','dst_theme_id','signal','relation_strength_mean','relation_edge_count','target_5m','target_15m','target_30m','target_60m','target_120m'],max_rg)
    if df.empty: rows=0
    else:
        if 'date' not in df: df['date']=part.date
        df['decision_time']=pd.to_datetime(df['decision_time'],utc=True); df['target_path_id']=path_id(df['dst_theme_id']); df['signal']=pd.to_numeric(df['signal'],errors='coerce').fillna(0.0)
        df['pos_signal']=df.signal.clip(lower=0); df['neg_signal']=df.signal.clip(upper=0); df['abs_signal']=df.signal.abs(); late=df.decision_time >= (df.decision_time.max()-pd.Timedelta(minutes=late_min)); df['late_signal']=df.signal.where(late,0.0); df['late_abs_signal']=df.abs_signal.where(late,0.0); df['obs']=1; df['pos_obs']=(df.signal>0).astype('int64'); df['neg_obs']=(df.signal<0).astype('int64')
        gcols=['date','layer_id','scale','level','target_path_id']; targets=[c for c in ['target_5m','target_15m','target_30m','target_60m','target_120m'] if c in df]
        res=df.groupby(gcols,sort=False).agg(first_time=('decision_time','min'),last_time=('decision_time','max'),observation_count=('obs','sum'),daily_pressure=('signal','sum'),positive_pressure=('pos_signal','sum'),negative_pressure=('neg_signal','sum'),absolute_pressure=('abs_signal','sum'),positive_observations=('pos_obs','sum'),negative_observations=('neg_obs','sum'),relation_edge_count_sum=('relation_edge_count','sum'),avg_relation_strength=('relation_strength_mean','mean'),late_signal_sum=('late_signal','sum'),late_abs_signal_sum=('late_abs_signal','sum')).reset_index()
        if targets: res=res.merge(df.groupby(gcols,sort=False)[targets].mean().reset_index().rename(columns={c:f'{c}_mean_proxy' for c in targets}),on=gcols,how='left')
        res['pressure_intensity']=res.daily_pressure/res.observation_count.replace(0,np.nan); res['positive_observation_rate']=res.positive_observations/res.observation_count.replace(0,np.nan); res['negative_observation_rate']=res.negative_observations/res.observation_count.replace(0,np.nan); res['persistence_proxy']=res.positive_observation_rate-res.negative_observation_rate; res['late_absolute_share']=res.late_abs_signal_sum/res.absolute_pressure.replace(0,np.nan); res['late_confirmation_score']=res.late_signal_sum*res.late_absolute_share.fillna(0)
        for c in ['daily_pressure','absolute_pressure','relation_edge_count_sum','late_confirmation_score']:
            std=res[c].std(ddof=0); res[c+'_z']=0.0 if not np.isfinite(std) or std==0 else (res[c]-res[c].mean())/std
        if 'target_5m_mean_proxy' in res:
            std=res.target_5m_mean_proxy.std(ddof=0); tz=0.0 if not np.isfinite(std) or std==0 else (res.target_5m_mean_proxy-res.target_5m_mean_proxy.mean())/std; res['target_underreaction_z']=res.daily_pressure_z-tz
        else: res['target_underreaction_z']=np.nan
        res['daily_pressure_score']=res.daily_pressure_z*res.persistence_proxy.fillna(0); res['daily_consensus_score']=res.daily_pressure_z*np.log1p(res.relation_edge_count_sum.clip(lower=0)); res['daily_underreaction_score']=res.daily_pressure_score*res.target_underreaction_z.fillna(0)*(1+res.late_absolute_share.fillna(0)); write_parquet_atomic(res,op); rows=len(res)
    meta={'stage':'daily_relation_features','status':'complete' if rows else 'empty','date':part.date,'layer_id':part.layer_id,'scale':part.scale,'output_rows':int(rows),'input':str(part.base),'output':str(op),'late_minutes':late_min,'elapsed_sec':round(time.time()-t,3)}; manifest(out,meta); return meta

def eval_daily(root,out_dir):
    t=time.time(); frames=[pd.read_parquet(p) for p in Path(root).rglob('daily_relation_features.parquet')]
    out=Path(out_dir); out.mkdir(parents=True,exist_ok=True)
    if not frames: meta={'stage':'daily_feature_eval','status':'empty','input_count':0}; manifest(out,meta); return meta
    df=pd.concat(frames,ignore_index=True); rows=[]; scores=[c for c in ['daily_pressure_score','daily_underreaction_score','daily_consensus_score','late_confirmation_score_z'] if c in df]; targets=[c for c in ['target_5m_mean_proxy','target_15m_mean_proxy','target_30m_mean_proxy','target_60m_mean_proxy'] if c in df]
    for sc in scores:
      for ta in targets:
        for keys,sub in df.groupby(['date','layer_id','scale','level'],dropna=False,sort=False):
            ...
            q8,q2=v[sc].quantile(.8),v[sc].quantile(.2); top=v[v[sc]>=q8][ta].mean(); bot=v[v[sc]<=q2][ta].mean(); rows.append({'date':keys[0],'layer_id':keys[1],'scale':keys[2],'level':keys[3],'score':sc,'target':ta,'sample_count':len(v),'rank_ic':v[sc].rank().corr(v[ta].rank()),'top_minus_bottom':top-bot})
    m=pd.DataFrame(rows); mp=out/'daily_alpha_metrics.csv'; sp=out/'daily_alpha_summary.csv'; m.to_csv(mp,index=False)
    s=m.groupby(['score','target','layer_id','scale','level'],sort=False).agg(days=('date','nunique'),sample_count=('sample_count','sum'),mean_rank_ic=('rank_ic','mean'),mean_spread=('top_minus_bottom','mean'),positive_day_rate=('top_minus_bottom',lambda x:float((x>0).mean()))).reset_index() if not m.empty else pd.DataFrame(); s.to_csv(sp,index=False)
    meta={'stage':'daily_feature_eval','status':'complete' if len(m) else 'empty','input_count':len(frames),'metric_rows':int(len(m)),'summary_rows':int(len(s)),'metrics':str(mp),'summary':str(sp),'elapsed_sec':round(time.time()-t,3)}; manifest(out,meta); return meta
