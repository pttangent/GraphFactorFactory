#!/usr/bin/env python3
"""Resource-safe P0 graph alpha CLI with direct canonical-date execution."""
...
def main() -> None:
    ...
    direct = commands.add_parser(
        "direct",
        help="scan each canonical date once and write final P0 factors without p0_alpha_shards",
    )
    direct.add_argument("--p0-root", required=True)
    direct.add_argument("--labels-root", required=True)
    direct.add_argument("--out-root", required=True)
    add_filters(direct)
    direct.set_defaults(workers=8)
    direct.add_argument("--features", default="node,spillover,graph")
    direct.add_argument("--batch-size", type=int, default=500_000)
    direct.add_argument("--min-free-gb", type=float, default=50.0)
    direct.add_argument("--disk-check-every", type=int, default=25)
    ...
    print(
        json.dumps(
            {"pit_contract_version": PIT_CONTRACT_VERSION, "result": result},
            indent=2,
            ensure_ascii=False,
            default=str,
        )
