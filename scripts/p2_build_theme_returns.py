...
def process_date(d, load_lock):
    try:
        ...
        if not mem_path.exists() or not lbl_path.exists():
            return f"[{d}] Missing files."
            
        # Serialize the massive memory allocation spikes during parquet reads
        with load_lock:
        ...
        if chunk_results:
            ...
            return f"[{d}] Done! ({len(final_rets)} rows)"
        
        return f"[{d}] No data yielded."
    except Exception as e:
        return f"[{d}] ERROR: {str(e)}"

if __name__ == '__main__':
    print("Building theme returns concurrently with Lock...", flush=True)
    m = multiprocessing.Manager()
    load_lock = m.Lock()
    
    with ProcessPoolExecutor(max_workers=6) as executor:
        futures = {executor.submit(process_date, d, load_lock): d for d in dates}
