...
class ThemeDiscoveryPipeline:
    ...
    def run(self, date_start=None, date_end=None):
        ...
        previous=[]; previous_records={}; outputs=[]
        for day in sorted((self.graph_root / "canonical").glob("date=*")):
            trade_date = day.name.split("=",1)[1]
            if date_start and trade_date < date_start: continue
            if date_end and trade_date > date_end: continue
            edges = pd.read_parquet(day / "edges.parquet")
            nodes = pd.read_parquet(day / "node_features.parquet")
            for snapshot_time, snapshot_edges in edges.groupby("decision_time", sort=True):
                layer_communities=[]; subcommunities=[]
                for layer_id, layer_edges in snapshot_edges.groupby("layer_id"):
                    ...
                    layer_communities.extend(parents); subcommunities.extend(children)
                ...
                target = self.store.write_snapshot(trade_date=trade_date, snapshot_time=snapshot_time, layer_communities=layer_communities, subcommunities=subcommunities, themes=candidates, lifecycle=lifecycle, semantics=semantics)
