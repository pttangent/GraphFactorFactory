...
from graphfactorfactory.application.return_corr_patch import ReturnCorrPatchPipeline
...
def main() -> None:
    ...
    patch = commands.add_parser(
        "patch-return-corr-date",
        help="Copy a baseline date into a new store and atomically rebuild ReturnCorr layers 1/14/15",
    )
    patch.add_argument("--node-factors", required=True)
    patch.add_argument("--date", required=True)
    patch.add_argument("--source-graph-store", required=True)
    patch.add_argument("--output-graph-store", required=True)
    patch.add_argument("--config", default="configs/default.yaml")
    patch.add_argument("--max-workers", type=int, default=1)
    ...
    if args.command == "build-date":
        ...
        payload = {
            ...
            "node_feature_rows": result.node_feature_rows,
            "snapshot_rows": result.snapshot_rows,
            "label_rows": result.label_rows,
        }
    elif args.command == "patch-return-corr-date":
        config = BuildConfig.from_yaml(args.config)
        source = ParquetNodeFactorSource(args.node_factors)
        payload = ReturnCorrPatchPipeline(
            source,
            args.source_graph_store,
            args.output_graph_store,
            config,
